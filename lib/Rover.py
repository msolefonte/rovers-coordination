...
class Rover(SDNNode):
    ...
    def _handle_message(self, message, client_address, connection):
        ...
        body = json.loads(message['body']), connection
        if body['type'] == 'heartbeat':
            connection.sendall(json.dumps(
                {'emitter': self.node_id, 'location': self.location, 'body': 'OK'}
            ).encode())
        elif body['type'] == 'target' and body['nonce'] not in self.consumed_nonces.keys():
            self.consumed_nonces[body['nonce']] = time.time()
            if body['to'] == self.node_id:
                print('[INFO] That body was for me, so nice! Thank you', body['reply_to'])
            else:
                ttl = body['ttl'] - 1
                if ttl >= 0:
                    self.send_body_to(body['body'], body['to'], body['reply_to'],
                                               body['nonce'], ttl)
